...
namespace web
{
	...
	static JsonValue makeBinaryRequest(const JsonValue& request)
	{
		JsonValue r = request;
		r.prop("_binary").set_boolean(true);
		return r;
	}
	...
	task<std::vector<uint8_t>> Native::queryBinary(const JsonValue& request)
	{
		...
		JSValue args = JSValue::object();
		args.set("request", makeBinaryRequest(request).to_json());
		args.set("persistent", false);
		args.set("onSuccess", successHandler->getHandler());
		args.set("onFailure", failureHandler->getHandler());
	...
	void Native::queryBinary(const JsonValue& request, std::function<void(std::vector<uint8_t>)> onSuccess, std::function<void(int code, std::string message)> onFailure)
	{
		auto successHandler = std::make_unique<JSCallback>([=](JSValue args) -> JSValue
		{
			if (onSuccess)
