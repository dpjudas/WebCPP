...
{
public:
	FileInputView(const std::string& filter, bool multiple, View* parent) : View(parent, "input")
	{
...
{
	auto fileDialog = new FileInputView(filter, multiple, DocumentBodyView::get());
...
{
	JSValue promise = handle.call<JSValue>("text");
	auto callback = new JSCallback([=](JSValue args) { onResult(vecFromJSArray<uint8_t>(args[0])); return JSValue::undefined(); }, true);
	promise.call<JSValue>("then", callback->getHandler());
}
