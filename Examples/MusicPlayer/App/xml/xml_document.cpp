...
std::shared_ptr<XmlDocument> XmlDocument::load(const std::string& input, bool eat_whitespace)
{
	...
	while (cur_token.type != XmlTokenType::null)
	{
		...
		case XmlTokenType::element:
			...
			{
				node_stack.pop_back();
				if (node_stack.empty()) throw std::runtime_error("Malformed XML tree!");
			}
			break;
...
void XmlDocumentImpl::save(std::function<void(const void* data, size_t size)> writeCallback, bool insert_whitespace) const
{
	auto writer = XmlWriter::create(writeCallback);
